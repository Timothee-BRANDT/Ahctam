                    ...
                    {responsiveFilterButton &&
                        <div className="sliders">
                            <div className='w-60 h-50 flex flex-col items-center gap-4'>
                                <div className="sliders-text text-1xl font-bold">Age - {age} {age > 1 ? 'years' : 'year'}</div>
                                <Slider value={[age]} onValueChange={handleSliderAgeChange} min={0} max={30} step={1} />
                                <div className="sliders-text text-1xl font-bold">Fame - {fame}</div>
                                <Slider value={[fame]} onValueChange={handleSliderFameChange} min={0} max={5} step={1} />
                            </div>
                            <div className='w-60 h-50 flex flex-col items-center gap-4'>
                                <div className="sliders-text text-1xl font-bold">Distance - {distance} {distance > 1 ? 'kms' : 'km'}</div>
                                <Slider value={[distance]} onValueChange={handleSliderDistanceChange} min={0} max={100} step={1} />
                                <div className="sliders-text text-1xl font-bold">Common Tags - {tags}</div>
                                <Slider value={[tags]} onValueChange={handleSliderTagsChange} min={0} max={20} step={1} />
                            </div>
                        </div>}
                    {responsiveFilterButton && <Button onClick={applyFilters} className="btn">Apply filters</Button>}
