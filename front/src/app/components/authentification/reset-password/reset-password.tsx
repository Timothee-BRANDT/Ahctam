...
const ResetPasswordPage: React.FC = () => {
  ...
  const submitUsername = async (event: any) => {
    ...
    try {
      const response = await fetch(
        `http://${serverIP}:5000/auth/forgot_password`,
        {
          method: "POST",
          headers: {
  ...
  const submitPassword = async (event: any) => {
    ...
    try {
      const response = await fetch(
        `http://${serverIP}:5000/auth/reset_password`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        <div className={`${CLASSNAME}__form-container`}>
          ...
          <form onSubmit={submitPassword} className="form">
            ...
            <div className={`${CLASSNAME}__helper`}>
              ...
              <Link className={`${CLASSNAME}__helper-link`} href="/login">
                Login !
              </Link>
            </div>
            {updateMessage && (
              <div className={`${CLASSNAME}__helper`}>
                <p className={`${CLASSNAME}__helper-password-updated`}>
